...
with open(os.path.join(base_dir, "kaggle_metadata.json"), "r") as f:
...
def test_mock_fetch_datasets(mock_response):
    ...
    with patch("croissant_rdf.kaggle.fetch_data.get_datasets", return_value=[mock_dataset]) as mock_get, patch(
        "croissant_rdf.kaggle.fetch_data.croissant_dataset",
...
def test_generate_ttl(mock_response):
    ...
    with patch("croissant_rdf.kaggle.fetch_data.croissant_dataset", return_value=mock_response.json()):
